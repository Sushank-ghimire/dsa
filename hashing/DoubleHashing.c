...
int secondHash(HashTable *ht, int key) {
  int R = ht->size - 1;
  return R - (key % R);
}
...
void insert(HashTable* ht) {
  ...
  int h1 = hashFunction(ht, key);
  int h2 = secondHash(ht, key);

  int i = 0;

  while(ht->table[(h1 + i * h2) % ht->size] != NULL) {
    i++;
    if(i == ht->size) {
  ...
  int insertIndex = (h1 + i * h2) % ht->size;
...
int search(HashTable* ht) {
  ...
  int i = 0;
  while (ht->table[(h1 + i * h2) % ht->size] != NULL) {
    if(ht->table[(h1 + i * h2) % ht->size]->key == key)
